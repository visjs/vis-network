...
*/
        ...
        if (
          typeof this.options.selfReference.angle !== "undefined" && this.options.selfReference.angle !== null
        ) {
          x = node1.x;
          y = node1.y;
          ...
          if(ctx){
            ...
            const toBorderDist = node1.distanceToBorder(ctx, this.options.selfReference.angle);
            ...
            //Hardcoding on which ranges are left, bottom, right 
            //Label position changed based on which side it is to avoid label overlapping with parent shape.
            if(this.options.selfReference.angle<=3.9 && this.options.selfReference.angle >=2.3){
              percentage = 0.375;//left
            }else if(this.options.selfReference.angle<=5.5 && this.options.selfReference.angle >=3.9){
              percentage = -0.375;//bottom
            }else if((this.options.selfReference.angle<=Math.PI*2 && this.options.selfReference.angle >=5.5) ||
            (this.options.selfReference.angle>=0 && this.options.selfReference.angle <=0.8)){
              percentage = 0.9;//right
            }
        ...
        } else if (node1.shape.width > node1.shape.height) {
        ...
        point = this._pointOnCircle(x, y, radius, percentage);
        this.labelModule.draw(ctx, point.x, point.y, this.selected, this.hover);
      }
    }
  }
