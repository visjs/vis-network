...
class Edge {
  ...
  static parseOptions(parentOptions, newOptions, allowDeletion = false, globalOptions = {}, copyFromGlobals = false) {
    ...
    if (newOptions.endPointOffset !== undefined) {
      if (ComponentUtil.isValidInteger(newOptions.endPointOffset.from)) {
        ...
        console.log('endPointOffset.from is not a valid number');
      }

      if (ComponentUtil.isValidInteger(newOptions.endPointOffset.to)) { 
        ...
        console.log('endPointOffset.to is not a valid number')
      }
    }
