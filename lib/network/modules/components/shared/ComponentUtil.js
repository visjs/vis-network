...
class ComponentUtil {
  ...
  /**
   * Check if given value is acceptable as an integer.
   *
   * @param {*} number value to check; can be anything at this point
   * @returns {boolean} true if valid integer value, false otherwise
   */
  static isValidInteger(number) {
    return (typeof number === 'number' && Number.isInteger(number) && Number.isFinite(number));
  }
