...
class Node {
  ...
  setOptions(options) {
    ...
    if (options.opacity !== undefined && Node.checkOpacity(options.opacity)) {
      ...
      // FIXME: This doesn't work if a group has already set this option
      this.options.opacity = options.opacity;
    }
    if (this.id >= 4 && this.id <= 7) {
      console.log(this.options, options.opacity, Node.checkOpacity(options.opacity));
    }
  ...
  static updateGroupOptions(parentOptions, newOptions, groupList) {
    if (this.id >= 4 && this.id <= 7) {
      console.log(newOptions, parentOptions)
    }
