...
class Image extends CircleImageBase {
  ...
  draw(ctx, x, y, selected, hover, values) {
    ...
    if (this.options.shapeProperties.useBorderWithImage === true) {
      ...
      ctx.strokeStyle = setColorOpacity(selected ? this.options.color.highlight.border : hover ? this.options.color.hover.border : this.options.color.border, values.opacity || 1);

      // set a fillstyle
      ctx.fillStyle = setColorOpacity(selected ? this.options.color.highlight.background : hover ? this.options.color.hover.background : this.options.color.background, values.opacity || 1);
