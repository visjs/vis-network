...
class InteractionHandler {
  ...
  constructor(body, canvas, selectionHandler) {
    ...
    // This array is used to keep track of selected nodes
    // before a drag operation starts. When a drag operation
    // finishes it is used to re-select the previously selected nodes
    this.previouslySelectedNodesBeforeDrag = [];
  }
  ...
  onDragStart(event) {
    ...
    if (node !== undefined && this.options.dragNodes === true) {
      ...
      if (node.isSelected() === false) {
        // Keep track of the previously selected nodes
        for (const selectedNode of this.selectionHandler.getSelectedNodes()) {
          this.previouslySelectedNodesBeforeDrag.push(selectedNode.id);
        }
  ...
  onDragEnd(event) {
    ...
    // If any previously selected node was deleted before the drag operation
    // we need to remove it from our array
    this.previouslySelectedNodesBeforeDrag =
      this.previouslySelectedNodesBeforeDrag.filter(
        (nodeId) => !!this.body.nodes[nodeId]
      );
    // Automatically reselect any remaining previously selected nodes when the drag event is finished
    if (this.previouslySelectedNodesBeforeDrag.length > 0) {
      this.selectionHandler.unselectAll();
      this.selectionHandler.setSelection({
        nodes: this.previouslySelectedNodesBeforeDrag,
      });
      this.previouslySelectedNodesBeforeDrag = [];
    }
  }
