...
class InteractionHandler {
  ...
  onDragEnd(event) {
    ...
    if (this.body.selectionBox.show) {
      ...
      const toBeSelectedNodes = Object.values(this.body.nodes).filter((node) => (
        node.x >= selectionBoxPosition.start.x && node.x <= selectionBoxPosition.end.x &&
        node.y >= selectionBoxPosition.start.y && node.y <= selectionBoxPosition.end.y 
      ));

      toBeSelectedNodes.forEach((node) => (this.selectionHandler.selectObject(node)))
