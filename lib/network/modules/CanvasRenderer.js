...
class CanvasRenderer {
  ...
  _drawNodes(ctx, alwaysShow = false) {
    const nodes = this.body.nodes;
    let nodeIndices;
    if(typeof this.canvas.options.fieldZIndex==='string'){
      const zindex = this.canvas.options.fieldZIndex;
      nodeIndices = this.body.data.nodes.get({order:(a,b) => parseInt(a[zindex])-parseInt(b[zindex])}).map(i=>i.id);
    }
    else {
      nodeIndices = this.body.nodeIndices;
    }
    let node;
    const selected = [];
    const hovered = [];
